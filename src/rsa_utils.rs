...
//         IronCryptError::KeySavingError(format!(
//             "Erreur lors de l'écriture de la clé privée : {}",
//             e
//         ))
//     })?;
//     std::fs::write(pub_path, public_pem).map_err(|e| {
//         IronCryptError::KeySavingError(format!(
//             "Erreur lors de l'écriture de la clé publique : {}",
//             e
//         ))
//     })?;
...
pub fn save_keys_to_files(
    ...
    // Convertir la clé privée en format PKCS#8 PEM
    let private_pem = private_key.to_pkcs8_pem(Default::default()).map_err(|e| {
        IronCryptError::KeySavingError(format!(
            "Erreur lors de la conversion de la clé privée : {}",
            e
        ))
    ...
    // Sauvegarder la clé privée au format PKCS#8
    std::fs::write(priv_path, private_pem).map_err(|e| {
        IronCryptError::KeySavingError(format!(
            "Erreur lors de l'écriture de la clé privée : {}",
            e
    ...
    // Sauvegarder la clé publique au format PKCS#1
    std::fs::write(pub_path, public_pem).map_err(|e| {
        IronCryptError::KeySavingError(format!(
            "Erreur lors de l'écriture de la clé publique : {}",
            e
